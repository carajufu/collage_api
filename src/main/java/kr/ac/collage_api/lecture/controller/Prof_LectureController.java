...
public class Prof_LectureController {
	...
	public String main(Model model, String estbllctreCode) {
		...
		List<LectureVO> lectureVO = lectureService.getAllCourses(estbllctreCode);
		log.info("allCourseList : {}", lectureVO);

		model.addAttribute("allCourseList", lectureVO);
		return "lecture/profLctreMain";
	}
	...
	public String estblDetail(@PathVariable("estbllctreCode") String estbllctreCode
		...
		LectureVO estblCourseInfo = lectureService.getEstblCourseById(estbllctreCode);

		// 강의 평가 전체 정보 조회 (LCTRE_EVL)
		List<LectureVO> evalList = lectureService.getLectureEvalByEstblCode(estbllctreCode);

		// 강의 시간표 조회 (LCTRE_TIMETABLE)
		List<LectureVO> timetableList = lectureService.getTimetableByEstblCode(estbllctreCode);

		// 강의 평가 상세 정보 조회(LCTRE_EVL_IEM)
		List<LectureVO> evlIem = lectureService.getEvlIem(lctreEvlInnb);
		...
		return "lecture/profLctreDetail";
	}

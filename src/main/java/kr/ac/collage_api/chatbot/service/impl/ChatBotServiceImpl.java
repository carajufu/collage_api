...
public class ChatBotServiceImpl implements ChatBotService {
    ...
    public String getAnswer(String msg, String loginId) {
        ...
        int codeStart = resp.indexOf("```");
        int codeEnd   = resp.lastIndexOf("```");
        if (codeStart != -1 && codeEnd > codeStart) {
            resp = resp.substring(codeStart + 3, codeEnd); 
        }
    ...
    private JSONObject buildStudentJson(String stdntNo) {
        ...
        // 성적
        SbjectScreVO score = chatBotMapper.getStudentSbjScore(stdntNo);
        if (score != null) {
        ...
        // 기본정보
        StdntVO info = chatBotMapper.getStudentInfo(stdntNo);
        if (info != null) {
...
//            i.put("major", info.getSubjctNm());
    ...
    private JSONObject buildProfessorJson(String profNo) {
        ...
        // 담당 강의 목록
        List<EstblCourseVO> lectureList = chatBotMapper.getProfessorLectureList(profNo);
...
//                o.put("name", l.getLctreNm());
        ...
        // 상담 예약 목록
        List<CnsltVO> cList = chatBotMapper.getProfessorCounselList(profNo);

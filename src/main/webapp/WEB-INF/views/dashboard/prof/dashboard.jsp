...
<!-- ================================================================== -->
<!-- [code-intent] 교수 대시보드: 담당 강의 + 캠퍼스 소식 + 학사 일정 캘린더 -->
<!-- [data-flow] 서버(Model) -> 카드/캘린더/테이블 UI -> 교수가 현재 강의/학사 일정 한눈에 파악 -->
<!-- ================================================================== -->
		                            ...
		                            <c:forEach items="${lectureList}" var="lecture">
		                                <div class="col">
		                                    ...
		                                    <div class="card card-body lecture-card-item js-lecture-card"
		                                         ...
		                                         data-current="${lecture.currentCnt}"
		                                         data-capacity="${lecture.capacityCnt}">
		                                        <!-- 강의명 -->
		                                        <h4 class="card-title mb-1"
		                                            data-key="${lecture.lctreNm}">
		                                            ${lecture.lctreNm}
		                                        </h4>
		
		                                        <!-- 강의실 -->
		                                        <p class="card-subtitle mb-1 text-muted"
		                                           data-key="${lecture.lctrum}">
		                                            강의실 ${lecture.lctrum}
		                                        </p>
		
		                                        <!-- 수강 인원 / 정원 -->
		                                        <c:set var="current" value="${lecture.currentCnt}" />
		                                        <c:set var="capacity" value="${lecture.capacityCnt}" />
		                                        ...
		                                        <!-- 간단 상태 배지: 여석 있음 / 만석 -->
		                                        <c:if test="${capacity > 0}">
		                                            <c:set var="ratio" value="${current * 100 / capacity}" />
		                                            <div class="mt-1">
		                                                <c:choose>
		                                                    <c:when test="${current >= capacity}">
		                                                        <span class="badge bg-danger-subtle text-danger">
		                                                            정원 마감
		                                                        </span>
		                                                    </c:when>
		                                                    <c:when test="${ratio >= 80}">
...
/* ======================================================================
 * 교수 대시보드 전용 JS
 * - initLectureCards  : 담당 강의 카드 클릭 -> 강의 상세/관리 페이지 이동
    ...
    function initLectureCards() {
        ...
        cards.forEach(card => {
            card.addEventListener("click", evt => {
                ...
                if (!lecNo) return;
                // 교수용 강의 상세 경로에 맞게 서버 라우팅만 조정하면 됨

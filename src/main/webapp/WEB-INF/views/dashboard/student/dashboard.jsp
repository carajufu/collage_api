...
<script
	src="${pageContext.request.contextPath}/js/fullcalendar/index.global.min.js"></script>
<!-- 전체 대시보드 래퍼: 좌우 패딩만, 세로 여백/간격 최소화 -->
<div class="row px-5 pb-4 g-0 align-items-start">
	...
	<div class="col-xxl-6 col-lg-6 dashboard-main-col">
		...
		<table class="table table-bordered align-middle text-center mb-3 academic-progress-table">
		    ...
		    <!-- 1) 총 이수학점 -->
		    <tr>
		        <td class="progress-metric-label">총 이수학점</td>
		        <td>
		            <div class="d-flex justify-content-between align-items-center mb-1">
		                <span class="progress-meta">
		                    ${totalCompletedCredits}/${totalRequiredCredits} 학점
		                    (${totalCreditRate}%)
		                </span>
		            </div>
		            <div class="progress" aria-label="총 이수학점 진행률">
		                <div class="progress-bar ${totalMet ? 'bg-primary' : 'bg-danger'}"
		                     role="progressbar"
		        ...
		        <td>
		            <span class="status-pill ${totalMet ? 'met' : 'not-met'}">
		                ${totalMet ? '충족' : '미충족'}
		            </span>
		        </td>
		    </tr>
		    ...
		    <!-- 2) 전공필수 -->
		    <tr>
		        <td class="progress-metric-label">전공필수 이수학점</td>
		        <td>
		            <div class="d-flex justify-content-between align-items-center mb-1">
		                <span class="progress-meta">
		                    ${majorCompletedCredits}/${majorRequiredCredits} 학점
		                    (${majorRequiredRate}%)
		                </span>
		            </div>
		            <div class="progress" aria-label="전공필수 이수학점 진행률">
		                <div class="progress-bar ${majorMet ? 'bg-primary' : 'bg-danger'}"
		                     role="progressbar"
		        ...
		        <td>
		            <span class="status-pill ${majorMet ? 'met' : 'not-met'}">
		                ${majorMet ? '충족' : '미충족'}
		            </span>
		        </td>
		    </tr>
		    ...
		    <!-- 3) 교양필수 -->
		    <tr>
		        <td class="progress-metric-label">교양필수 이수학점</td>
		        <td>
		            <div class="d-flex justify-content-between align-items-center mb-1">
		                <span class="progress-meta">
		                    ${liberalCompletedCredits}/${liberalRequiredCredits} 학점
		                    (${liberalRequiredRate}%)
		                </span>
		            </div>
		            <div class="progress" aria-label="교양필수 이수학점 진행률">
		                <div class="progress-bar ${liberalMet ? 'bg-primary' : 'bg-danger'}"
		                     role="progressbar"
		        ...
		        <td>
		            <span class="status-pill ${liberalMet ? 'met' : 'not-met'}">
		                ${liberalMet ? '충족' : '미충족'}
		            </span>
		        </td>
		    </tr>
		    ...
		    <!-- 4) 외국어 -->
		    <tr>
		        <td class="progress-metric-label">외국어 이수</td>
		        <td>
		            <div class="d-flex justify-content-between align-items-center mb-1">
		                <span class="progress-meta">
		                    ${foreignCompletedSubjects}/${foreignRequiredSubjects} 과목
		                    (${foreignRate}%)
		                </span>
		            </div>
		            <div class="progress" aria-label="외국어 이수 진행률">
		                <div class="progress-bar ${foreignMet ? 'bg-primary' : 'bg-danger'}"
		                     role="progressbar"
		        ...
		        <td>
		            <span class="status-pill ${foreignMet ? 'met' : 'not-met'}">
		                ${foreignMet ? '충족' : '미충족'}
		            </span>
		        </td>
		    </tr>
...
.calendar-container {
	margin: 0;
	padding: 0.5rem 1rem 0.75rem 1rem; /* 상단 패딩 약간 줄임 */
	background-color: #ffffff;
}
...
/* FullCalendar daygrid 이벤트 숨김(+more 제거) */
...
.calendar-container, .timetable-container {
	/* position: relative; */
	/* max-width: 1145px; */
	/* margin: 30px auto; */
	padding: 13px;
...
.dashboard-side-col {
	margin-top: 0;
	padding-top: 0.25rem;
	padding-left: 1.5rem; /* 좌측 여백, 필요하면 1.0rem~2.0rem 사이로 조절 */
}
/* 학적 이행 테이블 전용 스타일 */

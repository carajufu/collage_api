...
<<<<<<< HEAD
	...
	.then(data => {
		console.log(data);
		loadCourseList(data.estblCourseVOList);
	})
	.catch(error => {
	...
	function loadCourseList(list) { 
		...
		list.forEach(l=>{
			...
			html += `
					<tr>
						<td><input type="checkbox" name="checkbox" value="\${l.estbllctreCode}"></td>
...
<<<<<<< HEAD
		const selectedCodes = getCourseCodes();
=======
...
<<<<<<< HEAD
	...
	function getCourseCodes() {
			...
			const courseCodes = Array.from(checkboxs).map(checkbox => checkbox.value);

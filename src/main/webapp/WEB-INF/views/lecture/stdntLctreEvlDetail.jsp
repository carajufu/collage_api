    ...
    <div class="card-body p-4">
      <table class="table table-bordered align-middle mb-0">
        <tbody>
          ...
          <tr><th class="table-light">취득학점</th><td>${lectureInfo.acqsPnt}학점</td></tr>
  ...
  <div class="card shadow-sm">
    ...
    <div class="card-body p-4">
      ...
      <form id="evalForm">
        ...
        <!-- JSP 하드코딩 문항 정의 -->
        <c:set var="items"
               value="강의 내용이 명확했다.,실습 위주로 유익했다.,강의 속도가 적절했다.,질문에 대한 답변이 명확했다.,강의 준비가 충분했다." />

        <c:forEach var="item" items="${fn:split(items, ',')}" varStatus="vs">

          <div class="mb-4 pb-3 border-bottom eval-block">

            <!-- 문항 ID (1~5) -->
            <input type="hidden" class="evlInnb" value="${vs.count}" />

            <!-- 문항 내용 -->
            <p class="fw-semibold mb-3">${vs.count}. ${item}</p>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("submitBtn").addEventListener("click", () => {
    ...
    for (let i = 0; i < blocks.length; i++) {
      ...
      // 문항 번호 (1~5)
      const innb = block.querySelector(".evlInnb").value;
